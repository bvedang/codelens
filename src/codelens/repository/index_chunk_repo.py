...
def get_index_metadata(session: Session, repo_root: str | None) -> IndexMeta | None:
    if not repo_root:
        ...
        return None
    return session.exec(
        select(IndexMeta).where(col(IndexMeta.repo_root) == repo_root)
    ).first()
...
def get_index_chunks(session: Session, repo_root: str) -> dict[str, StoredChunk]:
    if not repo_root:
        ...
        return {}
    rows = session.exec(
        select(IndexChunk).where(IndexChunk.repo_root == repo_root)
    ).all()
    chunks = {}
    for chunk in rows:
...
def delete_index_chunks_by_prefix(
    session: Session, repo_root: str, chunk_id_prefix: str
) -> None:
    session.exec(
...
def upsert_index_build_status(
    session: Session, state: IndexBuildState
) -> IndexBuildState:
...
def get_index_build_status(session: Session, repo_root: str) -> IndexBuildState | None:
    result = session.exec(
        select(IndexBuildState).where(IndexBuildState.repo_root == repo_root)
    ).first()
