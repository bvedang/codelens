    ...
    return {
        ...
        "source_sets": {
            ...
            ":internal:main": {
                ...
                "external_jars": [],
                "output_dirs": [
                    str(base / "internal" / "build" / "classes" / "java" / "main")
                ],
            },
        },
    }
...
def test_visible_source_sets_include_transitive_project_dependencies(tmp_path):
    ...
    workspace_dict["source_sets"][":payments:main"]["project_dependencies"] = [
        ":internal:main"
    ]
    workspace = GradleWorkspaceModel.from_dict(workspace_dict)

    visible = workspace.visible_source_sets(
        SourceSetId(project_path=":orders", name="main")
    )
...
def test_visible_source_roots_include_generated_and_dependency_roots(tmp_path):
    ...
    roots = workspace.visible_source_roots(
        SourceSetId(project_path=":orders", name="main")
    )
...
def test_visible_external_binary_entries_exclude_project_artifact_jars(tmp_path):
    ...
    entries = workspace.visible_external_binary_entries(
        SourceSetId(project_path=":orders", name="main")
    )
    ...
    assert (
        str(tmp_path.resolve() / "payments" / "build" / "libs" / "payments.jar")
        not in entries
    )
